...
# Date: December 2021
# Purpose: Finish Preprocessing and run logistic regression and random forest classifiers on data with just process
#          name feature
####################################
...
# get only process name features
x = x['Process Name']
...
x = x.map(lambda e: -1 if e not in processes_set else e)

print("One Hot Encode Data Proc Name Feature")
proc_name_col = ['Process Name']
one_hot_encoder = OneHotEncoder(sparse=True, categories=processes.reshape(1,num_proc), handle_unknown='ignore')

# the reshape is only cuz it has one column, when you have more, won't need this
x = one_hot_encoder.fit_transform(x.values.reshape(-1, 1))
...
#########################
# Linear Regression/Random Forest Classifier On Only Process Name
#########################

# run models on 5 random users to get individual user graphs and statistics
lr_proc_n_va_acc_list, lr_proc_n_test_acc_list, lr_proc_n_va_roc_list, lr_proc_n_te_roc_list = \
    run_logistic_regression_users(target_users, "Process Name Feature", x_tr, x_va, x_te, y_tr, y_va,
                                  y_te, True, True, True)

np.savetxt("lr_proc_name_roc_va.csv", np.array(lr_proc_n_va_roc_list), delimiter=',')
np.savetxt("lr_proc_name_roc_te.csv", np.array(lr_proc_n_te_roc_list), delimiter=',')
np.savetxt("lr_proc_name_acc_va.csv", np.array(lr_proc_n_va_acc_list), delimiter=',')
np.savetxt("lr_proc_name_acc_te.csv", np.array(lr_proc_n_test_acc_list), delimiter=',')

rf_proc_n_va_acc_list, rf_proc_n_test_acc_list, rf_proc_n_va_roc_list, rf_proc_n_te_roc_list = \
    run_random_forest_users(target_users, "Process Name Feature", x_tr, x_va, x_te, y_tr, y_va, y_te, True, True, True)


np.savetxt("rf_proc_name_roc_va.csv", np.array(rf_proc_n_va_roc_list), delimiter=',')
np.savetxt("rf_proc_name_roc_te.csv", np.array(rf_proc_n_te_roc_list), delimiter=',')
np.savetxt("rf_proc_name_acc_va.csv", np.array(rf_proc_n_va_acc_list), delimiter=',')
np.savetxt("rf_proc_name_acc_te.csv", np.array(rf_proc_n_test_acc_list), delimiter=',')